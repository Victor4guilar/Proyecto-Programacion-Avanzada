...
class MetodosNumericosApp:
    ...
    def metodo_euler(
        ...
        """
        MÉTODO DE EULER (Primer Orden) con cálculo de error y solución analítica.
        """
        ejex = []
        ejey = []
        ejey_real = []
        ...
        # Diccionario seguro para evaluar funciones
        contexto_math = {
            "x": x,
            "y": y,
            "sin": math.sin,
        ...
        # Evaluar punto inicial analítico si existe
        if expr_analitica:
            contexto_math["x"] = x
            y_real = eval(expr_analitica, {"__builtins__": {}}, contexto_math)
            ejey_real.append(y_real)
            errores.append(0.0) # Error en el punto inicial es 0
        ...
        while x < xf:
            ...
            # Si hay solución analítica, calcular el valor real y el error
            if expr_analitica:
                contexto_math["x"] = x
                y_real = eval(expr_analitica, {"__builtins__": {}}, contexto_math)
                ejey_real.append(y_real)
                ...
                if tipo_error == "Absoluto":
                    ...
                    if abs(y_real) > 1e-12:
                        errores.append(error_abs / abs(y_real))
                    else:
                        errores.append(error_abs)
                        
                elif tipo_error == "Porcentual":
                    if abs(y_real) > 1e-12:
                        errores.append((error_abs / abs(y_real)) * 100)
                    else:
                        errores.append(error_abs * 100)
                else:
                    errores.append(error_abs)

        return ejex, ejey, ejey_real, errores
    ...
    def cargar_metodo(
        ...
        #---MÉTODO DE EULER---
            ...
            self.entry_analitica = ttk.Entry(
                self.frame_inputs,
                width=40
            )

            self.entry_analitica.grid(
                row=1,
                column=1,
                padx=5,
                pady=2
    ...
    def ejecutar_metodo(
        ...
        try:
            ...
            #---MÉTODO DE EULER (Primer Orden)---
                ...
                expr = self.entry_func.get()
                expr_analitica = self.entry_analitica.get()
                
                # Asegúrate de cambiar 'self.combo_error.get()' por la variable real de tu combobox de error
                tipo_error = self.combo_error.get() 

                x0 = float(self.entry_x0.get())
                y0 = float(self.entry_y0.get())
                xf = float(self.entry_xf.get())
                n = int(self.entry_n.get())

                # Ejecutar el cálculo matemático
                ejex, ejey, ejey_real, errores = self.metodo_euler(
                    expr,
                    x0,
                    y0,
                    xf,
                ...
                texto = (
                    f"MÉTODO: {metodo}\n"
                    + "-" * 30
                    + f"\nValor aproximado: y({xf}) = {ejey[-1]:.6f}\n"
                )
                
                if expr_analitica:
                    # Colocar el símbolo % solo si es Porcentual
                    unidad_error = "%" if tipo_error == "Porcentual" else ""
                    texto += f"Valor Real Analítico: y({xf}) = {ejey_real[-1]:.6f}\n"
                    texto += f"Error {tipo_error} Final: {errores[-1]:.6f} {unidad_error}\n"
                ...
                #Gráfica de Euler vs Solución Real (1 fila, 2 columnas, posición 1)
                ax1 = fig.add_subplot(121)
                ...
                if expr_analitica:
                    ax1.plot(
                        ...
                        linestyle='--',
                        color='red',
                        label='Solución Analítica (Real)'
                    )

                ax1.set_title("Aproximación vs Real")
                ax1.set_xlabel("x")
                ax1.set_ylabel("y")
                ax1.grid(True)
                ax1.legend()

                #Gráfica del Error (1 fila, 2 columnas, posición 2)
                ax2 = fig.add_subplot(122)
                
                if expr_analitica:
                    etiqueta_error = f"Error {tipo_error}"
                    if tipo_error == "Porcentual":
                        etiqueta_error += " (%)"
                    ...
                    ax2.plot(
                        ejex,
                        errores,
                        marker='s', #Marcador cuadrado para el error
                        color='purple',
                        label= 'Evolución del Error'
                    )
                    ax2.set_ylabel(etiqueta_error)
                else:
                    #Mensaje por si dejan la solución analítica en blanco
                    ax2.text(
                        0.5, 0.5, 
                        "Requiere Solución Analítica\npara calcular el error", 
                        ha='center', va='center', fontsize=10, color='gray'
                    )
                    ax2.set_ylabel("Error")

                ax2.set_title(f"Error {tipo_error} por Paso")
                ax2.set_xlabel("x")
                ax2.grid(True)
                ax2.legend()

                # Ajusta el espacio horizontal entre los dos paneles para que no se encima el texto
                fig.tight_layout()

                # --- Renderizado en el Canvas de Tkinter ---
                canvas = FigureCanvasTkAgg(
                    fig,
                    master=self.frame_graficas
                )
