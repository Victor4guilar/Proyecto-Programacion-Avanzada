...
class MetodosNumericosApp:
    VERSION = "v1.3.0"  # Versión actualizada
    ...
    def crear_layout(self):
        ...
        # Añadido "Secante" a la lista
        self.combo_metodos['values'] = ["Bisección", "Falsa Posición", "Newton Raphson", "Secante"]
    ...
    def metodo_secante(self, expr, x0, x1, erra):
        ...
        while error > erra and n < 100:
            ...
            x0, x1 = x1, x2 # Desplazamiento de puntos
            n += 1
            ejex.append(n); ejey.append(error)
    ...
    def cargar_metodo(self):
        ...
        # Campo común: f(x)
        ttk.Label(self.frame_inputs, text="f(x):").grid(row=0, column=0, padx=5)
        ...
        if metodo == "Newton Raphson":
            ...
            ttk.Label(self.frame_inputs, text="x0 (Punto 1):").grid(row=1, column=0)
            self.entry_a = ttk.Entry(self.frame_inputs)
            self.entry_a.grid(row=1, column=1)
            
            ttk.Label(self.frame_inputs, text="x1 (Punto 2):").grid(row=2, column=0)
            self.entry_b = ttk.Entry(self.frame_inputs)
            self.entry_b.grid(row=2, column=1)

        else: # Bisección y Falsa Posición
            ttk.Label(self.frame_inputs, text="Límite a:").grid(row=1, column=0)
            self.entry_a = ttk.Entry(self.frame_inputs)
            self.entry_a.grid(row=1, column=1)
            
            ttk.Label(self.frame_inputs, text="Límite b:").grid(row=2, column=0)
            self.entry_b = ttk.Entry(self.frame_inputs)
            self.entry_b.grid(row=2, column=1)
    ...
    def ejecutar_metodo(self):
        try:
            ...
            if metodo == "Bisección":
                ...
                res, err, ex, ey = self.metodo_biseccion(expr, a, b, erra)
            
            elif metodo == "Falsa Posición":
                a, b = float(self.entry_a.get()), float(self.entry_b.get())
                res, err, ex, ey = self.metodo_falsa_posicion(expr, a, b, erra)
                
            elif metodo == "Newton Raphson":
                x0 = float(self.entry_a.get())
                f_der = self.entry_fprima.get()
                res, err, ex, ey = self.metodo_newton_raphson(expr, f_der, x0, erra)
                
            elif metodo == "Secante":
                x0, x1 = float(self.entry_a.get()), float(self.entry_b.get())
                res, err, ex, ey = self.metodo_secante(expr, x0, x1, erra)
